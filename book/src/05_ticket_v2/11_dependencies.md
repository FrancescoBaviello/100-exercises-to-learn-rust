...
consistently used across different builds (_e.g. in CI). If you're working on a project with multiple developers,
...
You can also specify dependencies that are only needed for development—_i._e. they only get pulled in when you're
running `cargo test`.\
