...
are respected (_i._e. you can't set a `Ticket`'s title to an empty string).
...
Since `set_title` takes ownership of `self` (_i._e. it **consumes it**), we need to reassign the result to a variable.
