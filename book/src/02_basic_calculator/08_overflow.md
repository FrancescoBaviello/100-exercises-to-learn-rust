...
to explicitly request via the `--release` flag—_e.g. `cargo build --release` or `cargo run --release`.
...
> social media (_e.g. Reddit, Twitter) before realizing they haven't built their project in
> release mode.
